...
if (isset($_GET['club_id'])) {
    ...
    if ($check_result && mysqli_num_rows($check_result) > 0) {
        ...
        if (mysqli_query($conn, $update_sql)) {
            ...
            redirect('../admin/clubs.php?status=success&message=' . urlencode(($new_status ? 'ล็อกชุมนุมเรียบร้อยแล้ว' : 'ปลดล็อกชุมนุมเรียบร้อยแล้ว')));
            exit; // เพิ่ม exit หลัง redirect
        } else {
            // แสดงข้อผิดพลาด MySQL เพื่อการแก้ไขปัญหา (ลบออกในการใช้งานจริง)
            // echo "MySQL Error: " . mysqli_error($conn);
            redirect('../admin/clubs.php?status=error&message=' . urlencode('เกิดข้อผิดพลาดในการอัพเดทสถานะชุมนุม'));
            exit;
        }
    } else {
        redirect('../admin/clubs.php?status=error&message=' . urlencode('ไม่พบข้อมูลชุมนุม'));
        exit;
    }
} else {
    redirect('../admin/clubs.php?status=error&message=' . urlencode('ไม่ได้ระบุรหัสชุมนุม'));
    exit;
}
?>