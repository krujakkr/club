...
// ===== AJAX: ดึงรายชื่อสมาชิกชุมนุม =====
if (isset($_GET['ajax']) && $_GET['ajax'] === 'get_club_members') {
    header('Content-Type: application/json');
    $club_id_filter = (int)$_GET['club_id'];
    $chk = mysqli_query($conn, "SELECT club_id FROM clubs WHERE club_id = $club_id_filter AND teacher_id = '$teacher_id'");
    if (mysqli_num_rows($chk) === 0) {
    ...
    $sql = "SELECT student_id, firstname, lastname, grade_level, class_room, class_number
            FROM students WHERE club_id = $club_id_filter AND selection_status = 1
            ORDER BY grade_level, class_room, class_number";
    $res = mysqli_query($conn, $sql);
    $rows = [];
    while ($r = mysqli_fetch_assoc($res)) $rows[] = $r;
...
// ===== POST: ยกเลิกนักเรียนออกจากชุมนุม (JSON) =====
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'remove_student') {
    header('Content-Type: application/json');
    if (!$period_active) {
        echo json_encode(['success' => false, 'message' => 'ไม่อยู่ในช่วงเวลาที่อนุญาตให้แก้ไข']);
        exit;
    }
    $club_id_r    = (int)$_POST['club_id'];
    $student_id_r = mysqli_real_escape_string($conn, $_POST['student_id'] ?? '');
    $chk = mysqli_query($conn, "SELECT club_id FROM clubs WHERE club_id = $club_id_r AND teacher_id = '$teacher_id'");
    if (mysqli_num_rows($chk) === 0) {
        echo json_encode(['success' => false, 'message' => 'ไม่มีสิทธิ์จัดการชุมนุมนี้']);
        exit;
    }
    $upd = "UPDATE students SET club_id = NULL, selection_status = 0 WHERE student_id = '$student_id_r' AND club_id = $club_id_r";
    if (mysqli_query($conn, $upd) && mysqli_affected_rows($conn) > 0) {
        logActivity($teacher_id, 'teacher', 'remove_student', "ยกเลิกนักเรียน student_id=$student_id_r จาก club_id=$club_id_r");
        echo json_encode(['success' => true, 'message' => 'ยกเลิกนักเรียนออกจากชุมนุมเรียบร้อยแล้ว']);
    } else {
        echo json_encode(['success' => false, 'message' => 'ไม่พบนักเรียนในชุมนุมนี้']);
    }
    exit;
}
    ...
    <div class="card club-card mb-4">
        <div class="card-header bg-white d-flex justify-content-between align-items-center flex-wrap gap-2">
            ...
            <div class="d-flex align-items-center gap-2 flex-wrap">
                <button type="button"
                        class="badge badge-members px-3 py-2 btn"
                        id="membersBadge_<?php echo $club['club_id']; ?>"
                        data-max="<?php echo $club['max_members']; ?>"
                        onclick="openMembersModal(<?php echo $club['club_id']; ?>, '<?php echo htmlspecialchars(addslashes($club['club_name'])); ?>', <?php echo $club['current_members']; ?>, <?php echo $club['max_members']; ?>)"
                        title="คลิกเพื่อดูรายชื่อสมาชิก"
                        style="cursor:pointer;">
                    <i class="fas fa-user-check me-1"></i>
                    <span class="members-count"><?php echo $club['current_members']; ?>/<?php echo $club['max_members']; ?> คน</span>
                </button>
...
<!-- ===== Modal: รายชื่อสมาชิกชุมนุม ===== -->
<div class="modal fade" id="membersModal" tabindex="-1" aria-labelledby="membersModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header" style="background: linear-gradient(135deg,#1a6b3c,#2d9b63); color:#fff;">
                <h5 class="modal-title" id="membersModalLabel">
                    <i class="fas fa-users me-2"></i>รายชื่อสมาชิก: <span id="membersModalClubName"></span>
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <div class="mb-3 d-flex justify-content-between align-items-center gap-2 flex-wrap">
                    <span class="badge bg-success fs-6" id="membersTotalBadge">0 คน</span>
                    <input type="text" class="form-control form-control-sm"
                           id="memberSearch" placeholder="ค้นหาชื่อ / รหัส..."
                           oninput="filterMemberRows()" style="max-width:220px;">
                </div>
...
// ===== Members Modal =====
let membersClubId   = null;
let membersData     = [];
const periodActive  = <?php echo $period_active ? 'true' : 'false'; ?>;

function openMembersModal(clubId, clubName, curMem, maxMem) {
    membersClubId = clubId;
    document.getElementById('membersModalClubName').textContent = clubName;
    document.getElementById('membersContent').innerHTML =
        '<div class="text-center py-4"><i class="fas fa-spinner fa-spin me-2"></i>กำลังโหลด...</div>';
    document.getElementById('membersTotalBadge').textContent = curMem + '/' + maxMem + ' คน';
    document.getElementById('memberSearch').value = '';
    new bootstrap.Modal(document.getElementById('membersModal')).show();
    loadMembers(clubId, maxMem);
}

async function loadMembers(clubId, maxMem) {
    const res = await fetch(`?ajax=get_club_members&club_id=${clubId}`);
    ...
    membersData = data;
    document.getElementById('membersTotalBadge').textContent = data.length + (maxMem ? '/' + maxMem : '') + ' คน';

    if (data.length === 0) {
        document.getElementById('membersContent').innerHTML =
            '<div class="alert alert-warning text-center"><i class="fas fa-users-slash me-2"></i>ยังไม่มีสมาชิกในชุมนุมนี้</div>';
        return;
    }
...
function renderMembersTable(rows) {
    const actionCol = periodActive ? '<th style="width:70px">จัดการ</th>' : '';
    let html = `
    ...
    rows.forEach((m, i) => {
        const removeBtn = periodActive
            ? `<button class="btn btn-danger btn-sm py-0 px-2"
                       onclick="confirmRemove('${escHtml(m.student_id)}','${escHtml(m.firstname)} ${escHtml(m.lastname)}')"
                       title="ยกเลิกออกจากชุมนุม">
                   <i class="fas fa-user-minus"></i>
               </button>` : '';
        html += `<tr data-name="${escHtml(m.firstname)} ${escHtml(m.lastname)}" data-id="${escHtml(m.student_id)}">
            <td class="text-center row-num">${i+1}</td>
            <td class="small">${escHtml(m.student_id)}</td>
            <td>${escHtml(m.firstname)} ${escHtml(m.lastname)}</td>
            <td class="text-center">${escHtml(m.grade_level)}</td>
            <td class="text-center">${m.class_room}</td>
            <td class="text-center">${m.class_number}</td>
            ${periodActive ? `<td>${removeBtn}</td>` : ''}
        </tr>`;
    });
...
async function confirmRemove(studentId, studentName) {
    const result = await Swal.fire({
        title: 'ยืนยันการยกเลิก',
        html: `ยกเลิก <strong>${escHtml(studentName)}</strong> ออกจากชุมนุมนี้?`,
        icon: 'warning',
    ...
    fd.append('club_id',    membersClubId);
    ...
    if (data.success) {
        ...
        // อัพเดท badge ในการ์ด
        const badgeEl = document.getElementById(`membersBadge_${membersClubId}`);
        if (badgeEl) {
            const max  = parseInt(badgeEl.dataset.max);
            const prev = parseInt(badgeEl.querySelector('.members-count').textContent);
            const next = Math.max(0, prev - 1);
            badgeEl.querySelector('.members-count').textContent = next + '/' + max + ' คน';
        }
        // โหลดรายชื่อใหม่
        const maxMem = parseInt(document.getElementById(`membersBadge_${membersClubId}`)?.dataset.max || 0);
        await loadMembers(membersClubId, maxMem);
    } else {
        Swal.fire({ icon: 'error', title: 'เกิดข้อผิดพลาด', text: data.message, confirmButtonColor: '#dc3545' });
    }
}
</script>
</body>
