...
$active_menu = 'students'; // เปลี่ยนตามตารางด้านล่าง
...
// จัดการการเพิ่มนักเรียนรายคน
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_student') {
    $new_student_id = trim($_POST['new_student_id'] ?? '');
    $new_id_card    = mysqli_real_escape_string($conn, trim($_POST['new_id_card'] ?? ''));
    $new_firstname  = mysqli_real_escape_string($conn, trim($_POST['new_firstname'] ?? ''));
    $new_lastname   = mysqli_real_escape_string($conn, trim($_POST['new_lastname'] ?? ''));
    $new_grade      = mysqli_real_escape_string($conn, $_POST['new_grade_level'] ?? '');
    $new_room       = (int)($_POST['new_class_room'] ?? 0);
    $new_number     = (int)($_POST['new_class_number'] ?? 0);

    if (empty($new_student_id) || empty($new_firstname) || empty($new_lastname) || empty($new_grade)) {
        $_SESSION['error_message'] = "กรุณากรอกข้อมูลที่จำเป็นให้ครบถ้วน";
    } else {
        // ตรวจสอบรหัสนักเรียนซ้ำ
        $dup_stmt = $conn->prepare("SELECT student_id FROM students WHERE student_id = ?");
        $dup_stmt->bind_param('s', $new_student_id);
        $dup_stmt->execute();
        $dup_result = $dup_stmt->get_result()->fetch_assoc();
        $dup_stmt->close();

        if ($dup_result) {
            $_SESSION['error_message'] = "รหัสนักเรียน \"$new_student_id\" มีในระบบแล้ว กรุณาตรวจสอบอีกครั้ง";
        } else {
            $ins_stmt = $conn->prepare("INSERT INTO students (student_id, id_card, firstname, lastname, grade_level, class_room, class_number, selection_status) VALUES (?, ?, ?, ?, ?, ?, ?, 0)");
            $ins_stmt->bind_param('sssssii', $new_student_id, $new_id_card, $new_firstname, $new_lastname, $new_grade, $new_room, $new_number);
            if ($ins_stmt->execute()) {
                logActivity($_SESSION['user_id'], 'admin', 'add_student', "เพิ่มนักเรียน: $new_firstname $new_lastname ($new_student_id)");
                $_SESSION['success_message'] = "เพิ่มนักเรียน $new_firstname $new_lastname เรียบร้อยแล้ว";
            } else {
...
<body>
                    ...
                    <div class="btn-toolbar mb-2 mb-md-0">
                        <div class="btn-group me-2">
                            <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#addStudentModal">
                                <i class="fas fa-user-plus me-1"></i> เพิ่มนักเรียน
                            </button>
                    ...
                    <div class="card-body">
                        ...
                        <?php else: ?>
                            <div class="text-center py-4">
                                ...
                                <p class="text-muted">ลองค้นหาด้วยคำค้นอื่น หรือ <a href="import.php">นำเข้าข้อมูลใหม่</a></p>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <div class="modal-content">
                <div class="modal-header bg-success text-white">
                    <h5 class="modal-title" id="addStudentModalLabel">
                        <i class="fas fa-user-plus"></i> เพิ่มนักเรียนรายคน
                    </h5>
                ...
                <form method="POST" id="addStudentForm" novalidate>
                    <div class="modal-body">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <label for="new_id_card" class="form-label">เลขบัตรประชาชน</label>
                                <input type="text" class="form-control" id="new_id_card" name="new_id_card"
                                       maxlength="13" inputmode="numeric" placeholder="กรอก 13 หลัก (ไม่บังคับ)">
                                <div class="form-text">กรอกเลข 13 หลัก ไม่ต้องมีขีดคั่น</div>
                            </div>
                            <div class="col-md-6">
                                <label for="new_student_id" class="form-label">รหัสนักเรียน <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="new_student_id" name="new_student_id" required
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <label for="new_firstname" class="form-label">ชื่อ <span class="text-danger">*</span></label>
                                <input type="text" class="form-control" id="new_firstname" name="new_firstname" required
                                       placeholder="เช่น เด็กชายสมชาย / นายสมชาย">
                                <div class="invalid-feedback">กรุณากรอกชื่อ</div>
                            </div>
                            <div class="col-md-6">
                                <label for="new_lastname" class="form-label">นามสกุล <span class="text-danger">*</span></label>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-4">
                                <label for="new_grade_level" class="form-label">ระดับชั้น <span class="text-danger">*</span></label>
                                <select class="form-select" id="new_grade_level" name="new_grade_level" required>
                            ...
                            <div class="col-md-4">
                                <label for="new_class_room" class="form-label">ห้อง</label>
                                <input type="number" class="form-control" id="new_class_room" name="new_class_room"
                                       min="1" max="20" placeholder="เลขห้อง">
                            </div>
                            <div class="col-md-4">
                                <label for="new_class_number" class="form-label">เลขที่</label>
                                <input type="number" class="form-control" id="new_class_number" name="new_class_number"
                                       min="1" max="60" placeholder="เลขที่">
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-success">
                            <i class="fas fa-user-plus"></i> นำเข้า
                        </button>
                    </div>
                </form>
            </div>
                ...
                <form method="POST" id="editStudentForm">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-text">แสดงเฉพาะชุมนุมที่เปิดรับระดับชั้นของนักเรียนและยังไม่เต็ม</div>
                        </div>
    ...
    <script>
        ...
        function exportStudents() {
            ...
            let exportUrl = '../api/export_students.php?';
            const params = [];
        ...
        function loadStudentData(studentId) {
            fetch('../api/get_student.php?student_id=' + encodeURIComponent(studentId))
                ...
                .then(data => {
                    ...
                    // โหลดชุมนุมที่เปิดสำหรับระดับชั้นนี้และยังไม่เต็ม
                    loadAvailableClubs(data.grade_level || '', data.club_id || '');
                })
                .catch(error => {
        ...
        function loadAvailableClubs(gradeLevel, currentClubId) {
            const select = document.getElementById('edit_club_id');
            select.innerHTML = '<option value="">-- กำลังโหลด... --</option>';

            const params = new URLSearchParams({
                grade_level: gradeLevel,
                current_club_id: currentClubId || 0
            });

            fetch('../api/get_clubs_available.php?' + params.toString())
                .then(response => response.json())
                .then(clubs => {
                    select.innerHTML = '<option value="">-- ไม่เลือกชุมนุม --</option>';
                    clubs.forEach(club => {
                        const isCurrent = String(club.club_id) === String(currentClubId);
                        const teacherPart = club.teacher_name ? ` | ครู${club.teacher_name}` : '';
                        const label = `${club.club_name}${teacherPart} (${club.current_members}/${club.max_members} คน)`
                            + (isCurrent && club.current_members >= club.max_members ? ' - ชุมนุมปัจจุบัน (เต็ม)' : '');
                        const opt = new Option(label, club.club_id);
                        if (isCurrent) opt.selected = true;
                        select.appendChild(opt);
                    });

                    if (clubs.length === 0) {
                        select.innerHTML = '<option value="">-- ไม่มีชุมนุมที่เปิดรับสำหรับระดับชั้นนี้ --</option>';
                    }
                })
                .catch(() => {
        ...
        // เมื่อเปลี่ยนระดับชั้นในฟอร์ม ให้โหลดชุมนุมใหม่ด้วย
        document.getElementById('edit_grade_level').addEventListener('change', function() {
            const currentClubId = document.getElementById('edit_club_id').value;
            loadAvailableClubs(this.value, currentClubId);
        });

        // ตรวจสอบเลขบัตรประชาชน (edit)
        document.getElementById('edit_id_card').addEventListener('input', function() {
            this.value = this.value.replace(/[^0-9]/g, '');
            if (this.value.length > 13) this.value = this.value.slice(0, 13);
        });

        // ตรวจสอบเลขบัตรประชาชน (add)
        document.getElementById('new_id_card').addEventListener('input', function() {
            this.value = this.value.replace(/[^0-9]/g, '');
            if (this.value.length > 13) this.value = this.value.slice(0, 13);
        });

        // Validate form เพิ่มนักเรียน
