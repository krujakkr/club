    ...
    if (isset($_POST['clear_data_type'])) {
        ...
        switch ($clear_type) {
            ...
            case 'clear_clubs':
                ...
                break;
            case 'clear_teachers':
                $count_result   = mysqli_query($conn, "SELECT COUNT(*) as total FROM teachers");
                $total_teachers = mysqli_fetch_assoc($count_result)['total'];
                // ยกเลิกความเชื่อมโยงชุมนุมก่อนลบครู
                mysqli_query($conn, "UPDATE clubs SET teacher_id = NULL");
                if (mysqli_query($conn, "DELETE FROM teachers")) {
                    logActivity($_SESSION['user_id'], 'admin', 'clear_teachers', "ลบข้อมูลครู: $total_teachers รายการ");
                    $success_messages[] = "ลบข้อมูลครูทั้งหมดเรียบร้อยแล้ว ($total_teachers รายการ)";
                } else {
                    $error_messages[] = 'เกิดข้อผิดพลาด: ' . mysqli_error($conn);
                }
                break;
                            ...
                            <div class="card-body">
                                ...
                                <form method="post" id="teacherPeriodForm">
                                    ...
                                    <div class="mt-3 d-flex gap-2 flex-wrap">
                                        ...
                                        <a href="../teacher/login.php" target="_blank" class="btn btn-outline-success btn-sm">
                    ...
                    <form method="post" id="clearDataForm">
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="clear_data_type" id="clear_data_type" required>
                                ...
                                <option value="clear_teachers">ลบข้อมูลครูทั้งหมด (<?php echo number_format($teachers_stats['total']); ?> รายการ)</option>
                            </select>
                        </div>
                        <div class="mb-3">
