...
$active_menu = 'clubs'; // เปลี่ยนตามตารางด้านล่าง
                            ...
                            <table class="table table-striped table-bordered" id="clubsTable">
                                ...
                                <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td><?php echo nl2br(htmlspecialchars(mb_substr($row['description'], 0, 100, 'UTF-8'))) . (mb_strlen($row['description'], 'UTF-8') > 100 ? '...' : ''); ?></td>
                                            <td class="text-center">
                                                <a href="../api/toggle_club_lock.php?club_id=<?php echo $club_id; ?>" class="btn <?php echo $is_locked ? 'btn-warning' : 'btn-dark'; ?> btn-sm mb-1" title="<?php echo $is_locked ? 'ปลดล็อกชุมนุม' : 'ล็อกชุมนุม'; ?>">
                                                    <i class="fas <?php echo $is_locked ? 'fa-lock-open' : 'fa-lock'; ?>"></i>
                                                </a>
                                                <a href="../print/club_members.php?club_id=<?php echo $club_id; ?>" class="btn btn-secondary btn-sm mb-1" target="_blank" title="พิมพ์รายชื่อสมาชิก">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="addClubForm" method="post" action="../api/add_club.php">
                        <div class="mb-3">
    ...
    <div class="modal fade" id="editClubModal" tabindex="-1" aria-labelledby="editClubModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="editClubForm" method="post" action="../api/update_club.php">
    ...
    <script>
        ...
        function deleteClub(clubId, clubName, membersCount) {
            ...
            if (confirm('คุณแน่ใจหรือไม่ที่จะลบชุมนุม "' + clubName + '"?')) {
                window.location.href = '../api/delete_club.php?club_id=' + clubId;
            }
        }

        // ฟังก์ชันถอนการเลือกชุมนุมของนักเรียน
        function removeMember(studentId, clubId, btn) {
            if (!confirm('ยืนยันการถอนการเลือกชุมนุมของนักเรียนรหัส ' + studentId + ' ?')) return;
            ...
            $.ajax({
                ...
                type: 'POST',
                data: { student_id: studentId, club_id: clubId },
                dataType: 'json',
                success: function(res) {
                    if (res.success) {
                        var row = document.getElementById('member-row-' + studentId);
                        if (row) row.remove();
                    } else {
        ...
        function exportClubs() {
            ...
            window.open('../api/export_clubs.php', '_blank');
        }
