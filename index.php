...
<body>
            ...
            <div class="col-md-12">
                <div class="card shadow">
                    <div class="card-header bg-primary text-white">
                        ...
                        <h6 class="mb-0">ปีการศึกษา <?php echo htmlspecialchars($academic_year); ?> ภาคเรียนที่ <?php echo htmlspecialchars($semester); ?></h6>
                    </div>
                    <div class="card-body">
                                    ...
                                    <?php 
                                        ...
                                        <tr class="<?php echo $row_class; ?>">
                                            ...
                                            <td class="club-name-cell">
                                                ...
                                                <?php endif; ?>
                                                <!-- ปุ่มดูรายละเอียด + ปุ่มดูสมาชิก แสดงเฉพาะมือถือ -->
                                                <?php
                                                    $detail = [
                                                        'name'        => $row['club_name'],
                                                    ...
                                                    $detail_json = htmlspecialchars(json_encode($detail, JSON_UNESCAPED_UNICODE), ENT_QUOTES);
                                                ?>
                                                <button type="button" class="btn btn-warning btn-sm d-lg-none ms-1 py-0 px-1" onclick="viewClubDetail(<?php echo $detail_json; ?>)" title="ดูรายละเอียดชุมนุม">
                    ...
                    <div class="card-footer text-muted">
                        <div class="row">
                            ...
                            <div class="col-md-6 text-end d-flex gap-2 justify-content-end">
                                ...
                                <?php if (!isLoggedIn()): ?>
                                    <a href="/admin/index.php" class="btn btn-outline-primary btn-sm">สำหรับผู้ดูแลระบบ</a>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-warning">
                    <h5 class="modal-title" id="clubDetailModalLabel"></h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body" id="club-detail-content"></div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">ปิด</button>
                </div>
            </div>
    ...
    <script>
        ...
        function viewClubDetail(data) {
            var statusHtml;
            if (data.is_locked) {
                statusHtml = '<span class="badge bg-secondary"><i class="fas fa-lock me-1"></i>ถูกล็อก</span>';
            } else if (data.is_full) {
                statusHtml = '<span class="badge bg-danger fw-bold">เต็ม</span>';
            } else {
                statusHtml = '<span class="badge bg-success fw-bold">ว่าง ' + data.available + ' ที่</span>';
            }

            var html = '<table class="table table-borderless mb-0">'
                + '<tr><th style="width:38%">คำอธิบาย</th><td>' + data.description + '</td></tr>'
                + '<tr><th>สถานที่เรียน</th><td>' + data.location + '</td></tr>'
                + '<tr><th>ครูที่ปรึกษา</th><td>' + data.teacher + '</td></tr>'
                + '<tr><th>ระดับชั้นที่รับ</th><td><div class="grade-levels">' + data.grades + '</div></td></tr>'
                + '<tr><th>ลงทะเบียนแล้ว</th><td>' + data.registered + ' / ' + data.max_members + ' คน</td></tr>'
                + '<tr><th>สถานะ</th><td>' + statusHtml + '</td></tr>'
                + '</table>';

            document.getElementById('clubDetailModalLabel').textContent = data.name;
            document.getElementById('club-detail-content').innerHTML = html;
            new bootstrap.Modal(document.getElementById('clubDetailModal')).show();
        }
